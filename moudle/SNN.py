...
class SimpleNN(nn.Module):
    ...
    def forward(self, x):
        x = self.embedding(x)
        # print("After embedding:", x.shape)  # 应为 (batch_size, max_length, hidden_dim)
        x = x.view(x.size(0), -1)
        # print("After view:", x.shape)  # 应为 (batch_size, hidden_dim * max_length)
        x = torch.relu(self.fc1(x))
        # print("After fc1:", x.shape)  # 应为 (batch_size, hidden_dim)
        x = self.fc2(x)
        # print("After fc2:", x.shape)  # 应为 (batch_size, output_dim)
        return x
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Training and Validation LossV2')
plt.legend()
plt.savefig('training_validation_lossV2.png')
plt.show()
