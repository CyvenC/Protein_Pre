...
for epoch in range(num_epochs):
    ...
    if avg_train_loss < 0.5 and avg_valid_loss < 0.5 and spearman_corr > best_spearman:
        ...
        torch.save(model.state_dict(), '../output/Xception/best_xception1d_model.pth')
...
plt.title('Xception Spearman Correlation Over Epochs')
plt.legend()
plt.savefig('Xception_spearman_correlation.png')
plt.show()

# 加载Spearman指标最高的模型权重
model.load_state_dict(torch.load('../output/Xception/best_xception1d_model.pth'))
model.eval()
# 测试模型
total_test_loss = 0
all_mutations = []
...
results_df.to_csv('flu_Xception_pre.csv', index=False)
print("预测值与实际值已保存为 flu_Xception_pre.csv")
