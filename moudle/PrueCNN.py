...
model = SimpleCNN(input_dim, hidden_dim, output_dim, max_length)
...
for epoch in range(num_epochs):
    ...
    if avg_train_loss < 0.5 and avg_valid_loss < 0.5 and spearman_corr > best_spearman:
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('SimpleCNN Training and Validation Loss')
plt.legend()
plt.savefig('SimpleCNN_training_validation_loss.png')
plt.show()
...
plt.title('SimpleCNN Spearman Correlation Over Epochs')
plt.legend()
plt.savefig('SimpleCNN_spearman_correlation.png')
plt.show()

# 加载Spearman指标最高的模型权重
model.load_state_dict(torch.load('best_simplecnn_model.pth'))
model.eval()
...
results_df.to_csv('flu_SimpleCNN_pre.csv', index=False)
print("预测值与实际值已保存为 flu_SimpleCNN_pre.csv")
