...
class ResNet1D(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, max_length):
        ...
        self.layer1 = ResidualBlock(hidden_dim, hidden_dim, KERNEL_SIZE, STRIDE, PADDING)
        self.layer2 = ResidualBlock(hidden_dim, hidden_dim, KERNEL_SIZE, STRIDE, PADDING)
        self.layer3 = ResidualBlock(hidden_dim, hidden_dim, KERNEL_SIZE, STRIDE, PADDING)
        self.layer4 = ResidualBlock(hidden_dim, hidden_dim, KERNEL_SIZE, STRIDE, PADDING)
...
#             torch.save(model.state_dict(), '../output/resnet/wt_con_best_resnet1dV4_model.pth')
#             print(f'New con best model saved with Spearman: {spearman_corr:.4f}')
#         else:
#             torch.save(model.state_dict(), '../output/resnet/wt_best_resnet1d_model.pth')
...
# plt.xlabel('Epoch')
# plt.ylabel('Loss')
# plt.title('wt ResNet V4 Training and Validation Loss')
# plt.legend()
# plt.savefig('wt_ResNet_V4_training_validation_loss.png')
# plt.show()
#
# # 可视化Spearman相关系数
# plt.figure(figsize=(10, 5))
...
# plt.title('wt ResNet V4 Spearman Correlation Over Epochs')
# plt.legend()
# plt.savefig('wt_ResNet_V4_spearman_correlation.png')
# plt.show()

# 加载Spearman指标最高的模型权重
model.load_state_dict(torch.load('../output/resnet/wt_best_resnet1d_model.pth'))
model.eval()
# 测试模型
total_test_loss = 0
all_mutations = []
...
results_df.to_csv('wt_con_flu_ResNet_V4_pre.csv', index=False)
print("预测值与实际值已保存为 wt_con_flu_ResNet_V4_pre.csv")
