...
for epoch in range(num_epochs):
    ...
    if  spearman_corr > best_spearman:
        ...
        if avg_train_loss < 0.5 and avg_valid_loss < 0.5:
            torch.save(model.state_dict(), '../output/resnet/wt_con_best_resnet1d_model.pth')
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('wt ResNet Training and Validation Loss')
plt.legend()
plt.savefig('wt_ResNet_training_validation_loss.png')
plt.show()
...
plt.title('wt ResNet Spearman Correlation Over Epochs')
plt.legend()
plt.savefig('wt_ResNet_spearman_correlation.png')
plt.show()

# 加载Spearman指标最高的模型权重
model.load_state_dict(torch.load('../output/resnet/wt_con_best_resnet1d_model.pth'))
model.eval()
# 测试模型
total_test_loss = 0
all_mutations = []
...
results_df.to_csv('wt_con_flu_ResNet_pre.csv', index=False)
print("预测值与实际值已保存为 wt_con_flu_ResNet_pre.csv")
